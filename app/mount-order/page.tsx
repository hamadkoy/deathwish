...
/** A Death Wish / Officer / Guild Master vote carries three times the weight. */
const heavyRanks = ["Death Wish", "Officer", "Guild Master"];
const heavyWeight = 3;
...
function tallyRank(votes: Vote[], weight: (voterId: string) => number) {
  const weights: Record<PerformanceRank, number> = { S: 0, A: 0, B: 0, C: 0, F: 0 };
  ...
  return { rank: best === 0 ? ("C" as PerformanceRank) : winner, weights, totalWeight };
}
...
export default function MountOrderPage() {
  ...
  const [me, setMe] = useState<{ id: string; rank: string | null } | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    const [attendanceMap, profilesResult, mountResult, votesResult, meResult] =
      await Promise.all([
        ...
        supabase
          .from("mount_order")
          .select("user_id, note, received, mount_name, received_at, mount_image"),
        supabase.from("mount_votes").select("voter_id, target_id, rank"),
        auth.user
          ? supabase.from("profiles").select("guild_role").eq("user_id", auth.user.id).single()
          : Promise.resolve({ data: null as any }),
      ]);

    if (auth.user) setMe({ id: auth.user.id, rank: meResult?.data?.guild_role ?? null });
    ...
    if (mountResult.error) {
      const retry = await supabase.from("mount_order").select("user_id, note, received");
    ...
    const merged: MountRow[] = (profilesResult.data ?? [])
      ...
      .map((profile) => {
        ...
        return {
          ...
          receivedAt: mount?.received_at ?? null,
          mountImage: mount?.mount_image ?? null,
          profile,
        };
      });
  ...
  const matches = useCallback(
    (row: Scored) =>
      (row.profile.discord_name ?? "").toLowerCase().includes(query.trim().toLowerCase()),
    [query]
  );

  const filtered = useMemo(() => active.filter(matches), [active, matches]);
  const notedRows = useMemo(() => scored.filter(matches), [scored, matches]);
  const receivedRows = useMemo(() => received.filter(matches), [received, matches]);

  const myVoteFor = useCallback(
    (targetId: string) =>
      me ? votes.find((v) => v.voter_id === me.id && v.target_id === targetId)?.rank : undefined,
    [votes, me]
  );
  ...
  async function castVote(targetId: string, rank: PerformanceRank) {
    ...
    const next = votes.filter((v) => !(v.voter_id === me.id && v.target_id === targetId));
    next.push({ voter_id: me.id, target_id: targetId, rank });
    setVotes(next);

    const { error } = await supabase
      .from("mount_votes")
      .upsert({ voter_id: me.id, target_id: targetId, rank }, { onConflict: "voter_id,target_id" });
    ...
    say(`Voted ${rank} for ${profileById.get(targetId)?.discord_name ?? "player"}`);
  }
  ...
  async function clearVote(targetId: string) {
    ...
    const previous = votes;
    setVotes(votes.filter((v) => !(v.voter_id === me.id && v.target_id === targetId)));
  ...
  async function persist(row: Scored, patch: Partial<MountRow>) {
    ...
    const payload: Record<string, unknown> = {
      ...
      mount_name: next.mountName || null,
      received_at: next.received ? next.receivedAt ?? new Date().toISOString() : null,
    };
      ...
      prev.map((r) =>
        r.user_id === row.user_id
          ? {
              ...r,
              ...patch,
              receivedAt: payload.received_at as string | null,
            }
          : r
      )
    );
  ...
  async function setReceived(row: Scored, value: boolean) {
    if (await persist(row, { received: value, receivedAt: value ? null : null }))
      say(
        value
          ? `${row.profile.discord_name} has the mount`
          : `${row.profile.discord_name} back in the order`
      );
  }
  ...
  async function saveMountName(row: Scored, mountName: string) {
    if (await persist(row, { mountName })) say("Mount name saved");
  }
  ...
  return (
    <main className="moPage">
      ...
      <header className="moHero">
        ...
        <h1>
          <span className="moFlourish left" aria-hidden="true" />
          Mount Order
          <span className="moFlourish right" aria-hidden="true" />
        </h1>

        <p className="moBlurb">
          Attendance comes from the raid sheet. The guild votes on performance and officers
          write down why. Both add up to the total that decides the queue.
        </p>
        ...
        <div className="moLegend">
          {rankOrder.map((r) => (
            <span className="moLegendChip" key={r} title={rankMeaning[r]}>
              ...
              <b className={`v${r}`}>
                {performancePoints[r] > 0 ? "+" : performancePoints[r] === 0 ? "±" : ""}
                {performancePoints[r] === 0 ? "0" : performancePoints[r].toLocaleString()}
              </b>
            </span>
          ))}
        </div>
      ...
      <div className="moShell">
        ...
        <section className="moFrame moMain">
          ...
          {tab === "standings" && (
            <div className="moList">
              {filtered.length === 0 && <p className="moEmpty">No raider by that name.</p>}
              ...
              {filtered.map((row) => {
                ...
                return (
                  <article className={`moGroup ${open ? "open" : ""}`} key={row.user_id}>
                    <div
                    ...
                    >
                      <span className={`moHex h${place <= 3 ? place : 0}`}>
                        <b>{place}</b>
                      </span>

                      <span className="moRaider">
                        <img
                        ...
                        <span className="moWho">
                          ...
                          <i className="moRole">
                            <span className="moRoleMark" aria-hidden="true" />
                            {row.profile.guild_role ?? "Trial"}
                          </i>
                        </span>
                        {row.note.trim() && <span className="moNoteDot" title="Has a note" />}
                      </span>

                      <span className="moCell">
                        <span className="moCellLabel">Attendance</span>
                        <span className="moAtt">{row.attendance.toLocaleString()}</span>
                      </span>

                      <span className="moCell">
                        <span className="moCellLabel">Performance</span>
                        <span className={`moVerdict r${row.rank}`}>
                          <i>{row.rank}</i> · {rankMeaning[row.rank]}
                          {row.cast.length > 0 && (
                            <em>
                              {row.cast.length} {row.cast.length === 1 ? "vote" : "votes"}
                            </em>
                          )}
                        </span>
                      </span>
                      ...
                      <span className="moCell">
                        <span className="moCellLabel">Total</span>
                        <span className="moTot">{row.total.toLocaleString()}</span>
                      </span>

                      <span className="moRowActions">
                        <span className={`moTag ${row.note.trim() ? "hasNote" : ""}`}>
                          {row.note.trim() ? "Note" : "Add note"}
                        </span>
                        <span className={`moTag vote ${mine ? "voted" : ""}`}>
                          {mine ? `Voted ${mine}` : canVote ? "Vote" : "Votes"}
                        </span>
                      </span>

                      <span className={`moCaret ${open ? "up" : ""}`} aria-hidden="true" />
                    </div>
                      ...
                      <div className="moPanel">
                        ...
                        <div className="moCol">
                          <h4>
                            ...
                            <span className="moWeightNote">
                              Death Wish, Officer and Guild Master count ×{heavyWeight}
                            </span>
                          </h4>
                            ...
                            {row.cast.map((v) => {
                              ...
                              return (
                                <li key={v.voter_id}>
                                  ...
                                  {w > 1 && <span className="moWeight">×{w}</span>}
                                  <span className={`moChip mini r${v.rank}`}>{v.rank}</span>
                                </li>
                              );
                            })}
                          </ul>
                            ...
                            <div className="moSpread">
                              {rankOrder.map((r) => (
                                <span className="moSpreadRow" key={r}>
                                  <span className={`moChip mini r${r}`}>{r}</span>
                              ...
                              <p className="moQuiet tiny">
                                {row.totalWeight} weighted points · verdict {row.rank}
                              </p>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="moCol">
                          <h4>
                            Your verdict
                            {canVote && myWeight > 1 && (
                              <span className="moWeightNote">Your vote counts ×{myWeight}</span>
                            )}
                          </h4>
                          ...
                          {canVote ? (
                            ...
                            <p className="moQuiet">
                              Voting opens at Raider rank. You can still read every vote here.
                            </p>
                          )}

                          <h4 className="gap">Officer note</h4>
                          <NoteEditor
                            value={row.note}
                            readOnly={!canEdit}
                          ...
                          {canEdit && (
                            <button
                              className="moSolid mt"
                              onClick={() => setReceived(row, true)}
          ...
          {tab === "notes" && (
            <div className="moGrid">
              {notedRows.length === 0 && <p className="moEmpty">Nobody to show here yet.</p>}

              {notedRows.map((row) => (
                <article className="moCard" key={row.user_id}>
                  <div className="moRaider">
                    <img
                      src={row.profile.avatar_url ?? "/default-avatar.png"}
                      alt=""
                      loading="lazy"
                    />
                    <span className="moWho">
                      <b>{row.profile.discord_name ?? "Unknown"}</b>
                      <i className="moRole">
                        <span className="moRoleMark" aria-hidden="true" />
                        {row.profile.guild_role ?? "Trial"}
                    ...
                    <span className={`moChip r${row.rank}`}>{row.rank}</span>
                  </div>
          ...
          {tab === "received" && (
            <div className="moGrid">
              {receivedRows.length === 0 && (
                <p className="moEmpty">Nobody has the mount yet. The queue decides who's first.</p>
              )}
              ...
              {receivedRows.map((row) => (
                <article className="moCard" key={row.user_id}>
                  <div className="moRaider">
                    ...
                    <MountArt src={row.mountImage} />
                  </div>

                  <div className="moGot">
                    <span className="moGotLabel">Mount received</span>
                    <b>{row.mountName || "Not recorded"}</b>
                    <i>{formatDate(row.receivedAt) || "Date unknown"}</i>
                  </div>
                  ...
                  {canEdit && (
                    <div className="moCardBar">
                      <MountNameEditor
                        value={row.mountName}
                        saving={busyId === row.user_id}
                        onSave={(text) => saveMountName(row, text)}
                      />
                      <button
        ...
        <aside className="moFrame moRail">
          ...
          {received.length === 0 && (
            <p className="moQuiet center pad">Nobody yet. Top of the order is next.</p>
          )}
          ...
          <ul className="moRailList">
            {received.slice(0, 5).map((row) => (
              <li key={row.user_id}>
                ...
                <span className="moGot rail">
                  <span className="moGotLabel">Mount received</span>
                  <b>{row.mountName || "Not recorded"}</b>
                  <i>{formatDate(row.receivedAt)}</i>
                </span>
                <MountArt src={row.mountImage} />
              </li>
            ))}
      ...
      {toast && <div className={`moToast ${toast.bad ? "bad" : ""}`}>{toast.text}</div>}
...
/**
 * Mount artwork when an officer has set mount_image. With no image there is
 * nothing to show, so the frame stays neutral violet rather than inventing a
 * color — a hue seeded from the name reads as data the guild doesn't have.
 */
function MountArt({ src }: { src: string | null }) {
  return (
    <span className="moArt" aria-hidden="true">
      {src ? <img src={src} alt="" loading="lazy" /> : <span className="moSigil" />}
      <style jsx>{`
        .moArt {
          position: relative;
          flex: none;
          ...
          background: linear-gradient(140deg, rgba(109, 40, 217, 0.35), rgba(10, 2, 18, 0.9));
        }
        .moArt img {
          width: 100%;
          height: 100%;
  ...
  if (readOnly) {
    return (
      <p className="moNoteRead">
        {value.trim() || "No note yet. An officer will explain the letter here."}
        <style jsx>{`
          .moNoteRead {
            margin: 0;
            font-size: 11.5px;
...
const css = `
  ...
  .tl { top: 3px; left: 3px; border-top-width: 2px; border-left-width: 2px; }
  .tr { top: 3px; right: 3px; border-top-width: 2px; border-right-width: 2px; }
  .bl { bottom: 3px; left: 3px; border-bottom-width: 2px; border-left-width: 2px; }
  .br { bottom: 3px; right: 3px; border-bottom-width: 2px; border-right-width: 2px; }
  ...
  .moGroup:hover { border-color: rgba(245, 215, 110, 0.5); }
  .moGroup.open { border-color: rgba(245, 215, 110, 0.6); }

  .moRow {
    display: grid;
    grid-template-columns: 52px minmax(0, 1.1fr) 96px 168px 96px auto 14px;
    align-items: center;
    gap: 14px;
    padding: 10px 14px;
    cursor: pointer;
  ...
  .moRow.self { box-shadow: inset 2px 0 0 var(--mo-gold); }
  ...
  .moHex {
    ...
    background: linear-gradient(180deg, rgba(168, 85, 247, 0.55), rgba(109, 40, 217, 0.35));
  }
  ...
  .h1 { background: linear-gradient(180deg, #f7e3a1, #c9962f); }
  .h1 b { color: var(--mo-gold); }
  .h2 { background: linear-gradient(180deg, #ffffff, #8f9296); }
  .h2 b { color: #e6e6e6; }
  .h3 { background: linear-gradient(180deg, #e0a06a, #8a4c22); }
  .h3 b { color: #e0a06a; }
  ...
  /* ---------- cells ---------- */
  .moCell { display: grid; gap: 5px; justify-items: center; min-width: 0; }
  ...
  /* The letter is a filled badge; the pill around it stays dark so the
     meaning and vote count keep their contrast. */
  .moVerdict i {
    display: inline-grid;
    place-items: center;
    width: 18px;
  ...
  .moVerdict.rS { border-color: rgba(250, 204, 21, 0.5); }
  .moVerdict.rS i { background: linear-gradient(180deg, #facc15, #a16207); color: #1a1200; }
  .moVerdict.rA { border-color: rgba(56, 189, 248, 0.5); }
  .moVerdict.rA i { background: linear-gradient(180deg, #38bdf8, #0284c7); }
  .moVerdict.rB { border-color: rgba(34, 197, 94, 0.5); }
  .moVerdict.rB i { background: linear-gradient(180deg, #22c55e, #15803d); }
  .moVerdict.rC { border-color: rgba(156, 163, 175, 0.4); }
  .moVerdict.rC i { background: linear-gradient(180deg, #9ca3af, #4b5563); }
  .moVerdict.rF { border-color: rgba(239, 68, 68, 0.5); }
  .moVerdict.rF i { background: linear-gradient(180deg, #ef4444, #7f1d1d); }

  .moRowActions { display: flex; gap: 6px; }
  ...
  .moTag.hasNote { border-color: rgba(245, 215, 110, 0.6); color: var(--mo-gold); }
  .moTag.voted { border-color: rgba(245, 215, 110, 0.6); color: var(--mo-gold); }
  ...
  .moChip.mini { width: 19px; height: 17px; font-size: 9px; border-radius: 3px; }

  /* Scoped on purpose. A bare .rS also matched .moVerdict.rS and painted the
     whole pill solid, which is not what the letter chip's fill is for. */
  .moChip.rS, .moFill.rS, .moVoteBtn.rS {
    background: linear-gradient(180deg, #facc15, #a16207);
    color: #1a1200;
  }
  .moChip.rA, .moFill.rA, .moVoteBtn.rA { background: linear-gradient(180deg, #38bdf8, #0284c7); }
  .moChip.rB, .moFill.rB, .moVoteBtn.rB { background: linear-gradient(180deg, #22c55e, #15803d); }
  .moChip.rC, .moFill.rC, .moVoteBtn.rC { background: linear-gradient(180deg, #9ca3af, #4b5563); }
  .moChip.rF, .moFill.rF, .moVoteBtn.rF { background: linear-gradient(180deg, #ef4444, #7f1d1d); }
  ...
  .moVoteBtn:hover { opacity: 0.9; filter: none; transform: translateY(-2px); }
  ...
  .moCard .moRaider img { width: 34px; height: 34px; }
  .moCard .moChip { margin-left: auto; }
  ...
  .moGot.rail { width: 104px; flex: none; text-align: right; }
  ...
  .sm { padding: 5px 9px; font-size: 9px; flex: none; }
  .mt { margin-top: 8px; }
  ...
  .moSolid {
    ...
    transition: filter 0.15s linear, transform 0.15s ease;
  }

  .moSolid:hover:not(:disabled) { filter: brightness(1.2); transform: translateY(-1px); }
  .moSolid:disabled { opacity: 0.4; cursor: not-allowed; }
  ...
  @media (max-width: 1180px) {
    ...
    .moRow { grid-template-columns: 46px minmax(0, 1fr) 88px 150px 88px auto 14px; gap: 10px; }
  }
  ...
  @media (max-width: 860px) {
    ...
    .moRow .moCell:nth-of-type(1) { display: none; }
    .moRow .moRowActions { grid-column: 1 / -1; justify-content: flex-start; }
