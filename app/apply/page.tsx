...
const [bestMythicProgress, setBestMythicProgress] = useState("");
  ...
  async function addCharacter() {
    ...
    try {
      const res = await fetch(
        `https://raider.io/api/v1/characters/profile?region=eu&realm=${realm.trim()}&name=${name.trim()}&fields=gear,raid_progression,mythic_plus_scores_by_season_current,mythic_plus_best_runs`
      );
...
best_mythic_progress: bestMythicProgress,
       ...
       <form onSubmit={submitApplication}>
          <section className="card">
            ...
            <div className="addGrid">
              ...
              <button type="button" className="addBtn" onClick={addCharacter}>
...
/>
                ...
                <a href={character.warcraftlogs_url} target="_blank" className="logsBtn">
  VIEW WARCRAFT LOGS
</a>
              </div>
...
/>
<input
  placeholder="Best Mythic Progress"
  value={bestMythicProgress}
  onChange={(e) => setBestMythicProgress(e.target.value)}
/>
<textarea
...
<textarea
  placeholder="Why do you want to join Death Wish?"
  value={whyJoin}
...
.card {
  background-image: url("/applyy.png");
  background-size: cover;
...
input, textarea { text-shadow: 0 0 4px rgba(255,255,255,0.15); }
        input:focus,
        textarea:focus,
        select:focus {
        ...
        .submitBtn {
          ...
          background:
            linear-gradient(180deg, rgba(95, 29, 130, 0.95), rgba(35, 8, 50, 0.98));
          color: #fff2d8;
          font-family: Georgia, "Times New Roman", serif;
          font-weight: 900;
          cursor: pointer;
        ...
        .submitBtn:hover {
          ...
          border-color: #d6a84f;
          background:
            linear-gradient(180deg, rgba(120, 38, 165, 0.98), rgba(42, 9, 62, 1));
        }
...
.successPopup {
  ...
  border-radius: 24px;
  background:
    radial-gradient(circle at top, rgba(128, 35, 190, 0.3), transparent 45%),
    linear-gradient(180deg, #11111a, #030308);
  border: 1px solid rgba(214, 168, 79, 0.75);
  box-shadow: 0 0 55px rgba(168, 85, 247, 0.75);
}
