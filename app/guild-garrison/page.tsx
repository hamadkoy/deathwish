...
export default function GuildGarrisonPage() {
...
async function addCharacter() {
  ...
  if (characters.length >= 3) {
    setPopup({
      title: "Guild Garrison Limit",
      message: "You can only add 3 characters in Guild Garrison.",
    });
    return;
  }
...
bosses.forEach((boss: any) => {
  if (
    ...
    mythicBosses.push(boss.name);
      ...
      const { error } = await supabase.from("guild_characters").insert([
        {
...
async function setMainCharacter(id: string) {
  const { data: authData } = await supabase.auth.getUser();
...
async function updateCharacterSpec() {
  ...
  async function deleteCharacter(id: string) {
    const { error } = await supabase.from("guild_characters").delete().eq("id", id);
...
backgroundSize: currentTheme.size || "cover",
...
<div style={headerRow}>
  ...
  {isOwnProfile
    ? "Guild Garrison"
    : `${viewedProfile?.discord_name || "User"}'s Guild Garrison`}
</h1>
              <p style={subtitle}>Guild-only character list. This is separate from your normal My Garrison.</p>
...
const tableHead: React.CSSProperties = {
  display: "grid",
  gridTemplateColumns: "2fr 120px 100px 1fr 90px",
  padding: "14px 16px",
  fontSize: 11,
  color: "#d6bfff",
  fontWeight: 900,
